    ...
    private:
        bool check_property_type(uniform_type type) const noexcept {
            ...
            debug_.error("RENDER: unexpected property type:\n"
                "--> Type: %0\n"
                "--> Expected: %1",
                uniform_type_traits::to_string(type),
                uniform_type_traits::to_string(ui_.type));
            return false;
        }
    private:
        debug& debug_;
        ...
        const image& image)
    {
        ...
        if ( !is_pixel_supported(decl) ) {
            state_->dbg().error("RENDER: Failed to create texture:\n"
                ...
                "--> Pixel type: %0",
                pixel_declaration::pixel_type_traits::to_string(decl.type()));
            return nullptr;
        }
        ...
        index_buffer::usage usage)
    {
        ...
        if ( !is_index_supported(decl) ) {
            state_->dbg().error("RENDER: Failed to create index buffer:\n"
                ...
                "--> Index type: %0",
                index_declaration::index_type_traits::to_string(decl.type()));
            return nullptr;
        }
        ...
        v2u offset)
    {
        ...
        if ( tex->decl() != decl ) {
            state_->dbg().error("RENDER: Failed to update texture:\n"
                ...
                "--> Texture format: %0\n"
                "--> Image format: %1",
                pixel_declaration::pixel_type_traits::to_string(tex->decl().type()),
                pixel_declaration::pixel_type_traits::to_string(decl.type()));
